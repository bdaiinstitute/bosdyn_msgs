// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from bosdyn_msgs:msg/Dock.idl
// generated code does not contain a copyright notice
#include "bosdyn_msgs/msg/detail/dock__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `docked_waypoint_id`
#include "rosidl_runtime_c/string_functions.h"
// Member `target_prep_pose`
#include "bosdyn_msgs/msg/detail/target__functions.h"
// Member `prompt_duration`
#include "builtin_interfaces/msg/detail/duration__functions.h"

bool
bosdyn_msgs__msg__Dock__init(bosdyn_msgs__msg__Dock * msg)
{
  if (!msg) {
    return false;
  }
  // dock_id
  // docked_waypoint_id
  if (!rosidl_runtime_c__String__init(&msg->docked_waypoint_id)) {
    bosdyn_msgs__msg__Dock__fini(msg);
    return false;
  }
  // target_prep_pose
  if (!bosdyn_msgs__msg__Target__init(&msg->target_prep_pose)) {
    bosdyn_msgs__msg__Dock__fini(msg);
    return false;
  }
  // target_prep_pose_is_set
  // prompt_duration
  if (!builtin_interfaces__msg__Duration__init(&msg->prompt_duration)) {
    bosdyn_msgs__msg__Dock__fini(msg);
    return false;
  }
  // prompt_duration_is_set
  return true;
}

void
bosdyn_msgs__msg__Dock__fini(bosdyn_msgs__msg__Dock * msg)
{
  if (!msg) {
    return;
  }
  // dock_id
  // docked_waypoint_id
  rosidl_runtime_c__String__fini(&msg->docked_waypoint_id);
  // target_prep_pose
  bosdyn_msgs__msg__Target__fini(&msg->target_prep_pose);
  // target_prep_pose_is_set
  // prompt_duration
  builtin_interfaces__msg__Duration__fini(&msg->prompt_duration);
  // prompt_duration_is_set
}

bool
bosdyn_msgs__msg__Dock__are_equal(const bosdyn_msgs__msg__Dock * lhs, const bosdyn_msgs__msg__Dock * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // dock_id
  if (lhs->dock_id != rhs->dock_id) {
    return false;
  }
  // docked_waypoint_id
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->docked_waypoint_id), &(rhs->docked_waypoint_id)))
  {
    return false;
  }
  // target_prep_pose
  if (!bosdyn_msgs__msg__Target__are_equal(
      &(lhs->target_prep_pose), &(rhs->target_prep_pose)))
  {
    return false;
  }
  // target_prep_pose_is_set
  if (lhs->target_prep_pose_is_set != rhs->target_prep_pose_is_set) {
    return false;
  }
  // prompt_duration
  if (!builtin_interfaces__msg__Duration__are_equal(
      &(lhs->prompt_duration), &(rhs->prompt_duration)))
  {
    return false;
  }
  // prompt_duration_is_set
  if (lhs->prompt_duration_is_set != rhs->prompt_duration_is_set) {
    return false;
  }
  return true;
}

bool
bosdyn_msgs__msg__Dock__copy(
  const bosdyn_msgs__msg__Dock * input,
  bosdyn_msgs__msg__Dock * output)
{
  if (!input || !output) {
    return false;
  }
  // dock_id
  output->dock_id = input->dock_id;
  // docked_waypoint_id
  if (!rosidl_runtime_c__String__copy(
      &(input->docked_waypoint_id), &(output->docked_waypoint_id)))
  {
    return false;
  }
  // target_prep_pose
  if (!bosdyn_msgs__msg__Target__copy(
      &(input->target_prep_pose), &(output->target_prep_pose)))
  {
    return false;
  }
  // target_prep_pose_is_set
  output->target_prep_pose_is_set = input->target_prep_pose_is_set;
  // prompt_duration
  if (!builtin_interfaces__msg__Duration__copy(
      &(input->prompt_duration), &(output->prompt_duration)))
  {
    return false;
  }
  // prompt_duration_is_set
  output->prompt_duration_is_set = input->prompt_duration_is_set;
  return true;
}

bosdyn_msgs__msg__Dock *
bosdyn_msgs__msg__Dock__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bosdyn_msgs__msg__Dock * msg = (bosdyn_msgs__msg__Dock *)allocator.allocate(sizeof(bosdyn_msgs__msg__Dock), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(bosdyn_msgs__msg__Dock));
  bool success = bosdyn_msgs__msg__Dock__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
bosdyn_msgs__msg__Dock__destroy(bosdyn_msgs__msg__Dock * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    bosdyn_msgs__msg__Dock__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
bosdyn_msgs__msg__Dock__Sequence__init(bosdyn_msgs__msg__Dock__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bosdyn_msgs__msg__Dock * data = NULL;

  if (size) {
    data = (bosdyn_msgs__msg__Dock *)allocator.zero_allocate(size, sizeof(bosdyn_msgs__msg__Dock), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = bosdyn_msgs__msg__Dock__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        bosdyn_msgs__msg__Dock__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
bosdyn_msgs__msg__Dock__Sequence__fini(bosdyn_msgs__msg__Dock__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      bosdyn_msgs__msg__Dock__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

bosdyn_msgs__msg__Dock__Sequence *
bosdyn_msgs__msg__Dock__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bosdyn_msgs__msg__Dock__Sequence * array = (bosdyn_msgs__msg__Dock__Sequence *)allocator.allocate(sizeof(bosdyn_msgs__msg__Dock__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = bosdyn_msgs__msg__Dock__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
bosdyn_msgs__msg__Dock__Sequence__destroy(bosdyn_msgs__msg__Dock__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    bosdyn_msgs__msg__Dock__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
bosdyn_msgs__msg__Dock__Sequence__are_equal(const bosdyn_msgs__msg__Dock__Sequence * lhs, const bosdyn_msgs__msg__Dock__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!bosdyn_msgs__msg__Dock__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
bosdyn_msgs__msg__Dock__Sequence__copy(
  const bosdyn_msgs__msg__Dock__Sequence * input,
  bosdyn_msgs__msg__Dock__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(bosdyn_msgs__msg__Dock);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    bosdyn_msgs__msg__Dock * data =
      (bosdyn_msgs__msg__Dock *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!bosdyn_msgs__msg__Dock__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          bosdyn_msgs__msg__Dock__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!bosdyn_msgs__msg__Dock__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
