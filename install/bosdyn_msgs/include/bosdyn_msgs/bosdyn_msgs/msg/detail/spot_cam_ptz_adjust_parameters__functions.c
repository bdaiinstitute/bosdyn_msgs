// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from bosdyn_msgs:msg/SpotCamPtzAdjustParameters.idl
// generated code does not contain a copyright notice
#include "bosdyn_msgs/msg/detail/spot_cam_ptz_adjust_parameters__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `localization_varname`
// Member `waypoint_id`
#include "rosidl_runtime_c/string_functions.h"
// Member `waypoint_tform_body`
#include "geometry_msgs/msg/detail/pose__functions.h"

bool
bosdyn_msgs__msg__SpotCamPtzAdjustParameters__init(bosdyn_msgs__msg__SpotCamPtzAdjustParameters * msg)
{
  if (!msg) {
    return false;
  }
  // localization_varname
  if (!rosidl_runtime_c__String__init(&msg->localization_varname)) {
    bosdyn_msgs__msg__SpotCamPtzAdjustParameters__fini(msg);
    return false;
  }
  // waypoint_id
  if (!rosidl_runtime_c__String__init(&msg->waypoint_id)) {
    bosdyn_msgs__msg__SpotCamPtzAdjustParameters__fini(msg);
    return false;
  }
  // waypoint_tform_body
  if (!geometry_msgs__msg__Pose__init(&msg->waypoint_tform_body)) {
    bosdyn_msgs__msg__SpotCamPtzAdjustParameters__fini(msg);
    return false;
  }
  // waypoint_tform_body_is_set
  return true;
}

void
bosdyn_msgs__msg__SpotCamPtzAdjustParameters__fini(bosdyn_msgs__msg__SpotCamPtzAdjustParameters * msg)
{
  if (!msg) {
    return;
  }
  // localization_varname
  rosidl_runtime_c__String__fini(&msg->localization_varname);
  // waypoint_id
  rosidl_runtime_c__String__fini(&msg->waypoint_id);
  // waypoint_tform_body
  geometry_msgs__msg__Pose__fini(&msg->waypoint_tform_body);
  // waypoint_tform_body_is_set
}

bool
bosdyn_msgs__msg__SpotCamPtzAdjustParameters__are_equal(const bosdyn_msgs__msg__SpotCamPtzAdjustParameters * lhs, const bosdyn_msgs__msg__SpotCamPtzAdjustParameters * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // localization_varname
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->localization_varname), &(rhs->localization_varname)))
  {
    return false;
  }
  // waypoint_id
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->waypoint_id), &(rhs->waypoint_id)))
  {
    return false;
  }
  // waypoint_tform_body
  if (!geometry_msgs__msg__Pose__are_equal(
      &(lhs->waypoint_tform_body), &(rhs->waypoint_tform_body)))
  {
    return false;
  }
  // waypoint_tform_body_is_set
  if (lhs->waypoint_tform_body_is_set != rhs->waypoint_tform_body_is_set) {
    return false;
  }
  return true;
}

bool
bosdyn_msgs__msg__SpotCamPtzAdjustParameters__copy(
  const bosdyn_msgs__msg__SpotCamPtzAdjustParameters * input,
  bosdyn_msgs__msg__SpotCamPtzAdjustParameters * output)
{
  if (!input || !output) {
    return false;
  }
  // localization_varname
  if (!rosidl_runtime_c__String__copy(
      &(input->localization_varname), &(output->localization_varname)))
  {
    return false;
  }
  // waypoint_id
  if (!rosidl_runtime_c__String__copy(
      &(input->waypoint_id), &(output->waypoint_id)))
  {
    return false;
  }
  // waypoint_tform_body
  if (!geometry_msgs__msg__Pose__copy(
      &(input->waypoint_tform_body), &(output->waypoint_tform_body)))
  {
    return false;
  }
  // waypoint_tform_body_is_set
  output->waypoint_tform_body_is_set = input->waypoint_tform_body_is_set;
  return true;
}

bosdyn_msgs__msg__SpotCamPtzAdjustParameters *
bosdyn_msgs__msg__SpotCamPtzAdjustParameters__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bosdyn_msgs__msg__SpotCamPtzAdjustParameters * msg = (bosdyn_msgs__msg__SpotCamPtzAdjustParameters *)allocator.allocate(sizeof(bosdyn_msgs__msg__SpotCamPtzAdjustParameters), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(bosdyn_msgs__msg__SpotCamPtzAdjustParameters));
  bool success = bosdyn_msgs__msg__SpotCamPtzAdjustParameters__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
bosdyn_msgs__msg__SpotCamPtzAdjustParameters__destroy(bosdyn_msgs__msg__SpotCamPtzAdjustParameters * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    bosdyn_msgs__msg__SpotCamPtzAdjustParameters__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
bosdyn_msgs__msg__SpotCamPtzAdjustParameters__Sequence__init(bosdyn_msgs__msg__SpotCamPtzAdjustParameters__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bosdyn_msgs__msg__SpotCamPtzAdjustParameters * data = NULL;

  if (size) {
    data = (bosdyn_msgs__msg__SpotCamPtzAdjustParameters *)allocator.zero_allocate(size, sizeof(bosdyn_msgs__msg__SpotCamPtzAdjustParameters), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = bosdyn_msgs__msg__SpotCamPtzAdjustParameters__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        bosdyn_msgs__msg__SpotCamPtzAdjustParameters__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
bosdyn_msgs__msg__SpotCamPtzAdjustParameters__Sequence__fini(bosdyn_msgs__msg__SpotCamPtzAdjustParameters__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      bosdyn_msgs__msg__SpotCamPtzAdjustParameters__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

bosdyn_msgs__msg__SpotCamPtzAdjustParameters__Sequence *
bosdyn_msgs__msg__SpotCamPtzAdjustParameters__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bosdyn_msgs__msg__SpotCamPtzAdjustParameters__Sequence * array = (bosdyn_msgs__msg__SpotCamPtzAdjustParameters__Sequence *)allocator.allocate(sizeof(bosdyn_msgs__msg__SpotCamPtzAdjustParameters__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = bosdyn_msgs__msg__SpotCamPtzAdjustParameters__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
bosdyn_msgs__msg__SpotCamPtzAdjustParameters__Sequence__destroy(bosdyn_msgs__msg__SpotCamPtzAdjustParameters__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    bosdyn_msgs__msg__SpotCamPtzAdjustParameters__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
bosdyn_msgs__msg__SpotCamPtzAdjustParameters__Sequence__are_equal(const bosdyn_msgs__msg__SpotCamPtzAdjustParameters__Sequence * lhs, const bosdyn_msgs__msg__SpotCamPtzAdjustParameters__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!bosdyn_msgs__msg__SpotCamPtzAdjustParameters__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
bosdyn_msgs__msg__SpotCamPtzAdjustParameters__Sequence__copy(
  const bosdyn_msgs__msg__SpotCamPtzAdjustParameters__Sequence * input,
  bosdyn_msgs__msg__SpotCamPtzAdjustParameters__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(bosdyn_msgs__msg__SpotCamPtzAdjustParameters);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    bosdyn_msgs__msg__SpotCamPtzAdjustParameters * data =
      (bosdyn_msgs__msg__SpotCamPtzAdjustParameters *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!bosdyn_msgs__msg__SpotCamPtzAdjustParameters__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          bosdyn_msgs__msg__SpotCamPtzAdjustParameters__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!bosdyn_msgs__msg__SpotCamPtzAdjustParameters__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
